...
Elevator::Elevator(int id, int startFloor): id_(id), currentFloor_(startFloor) {}
...
void Elevator::assign(const Request& request) {
    ...
    std::cout << "[Elevator " << id_ << "] Assigned target floor " << request.floor << '\n';
}
...
void Elevator::process() {
    while (!targets_.empty()) {
        ...
        auto wait = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - r.createdAt);
